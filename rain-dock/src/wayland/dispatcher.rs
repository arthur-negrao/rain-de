...
use crate::{
    shared::{
        ...
        event::{PendingEvent, WaylandEvent, WaylandEventType},
    },
    wayland::{handlers::handle_wayland_command, wayland_source::WaylandSource},
};

#[derive(Debug)]
pub struct WaylandState {
    pub(super) pending_events: HashMap<ZwlrForeignToplevelHandleV1, PendingEvent>,
    pub(super) events_sender: Sender<WaylandEvent>,
    pub(super) seat: Option<WlSeat>,
}

impl WaylandState {
    pub fn new(events_sender: Sender<WaylandEvent>, commands_receiver: Channel<WaylandCommand>) {
        thread::Builder::new()
    ...
    fn event(
        ...
        match event {
            ...
            Event::Closed => {
                if let Some(window) = state.pending_events.get_mut(proxy) {
                    // cleanup the window
                    window.event_type = WaylandEventType::Closed;
                }
            }
            Event::Done => {
